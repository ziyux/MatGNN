...
# Original version from: https://github.com/divelab/DIG/blob/dig/dig/threedgraph/method/spherenet/features.py
# A Fixed Version by Ziyu Xiang 12/20/2021
...
def bessel_basis(n, k, cutoff):
    ...
    for order in range(n):
        ...
        for i in range(k):
            ...
            ###############################
            normalizer_tmp += [sym.sqrt(2/Jn(zeros[order, i], order + 1)**2 / cutoff**3)]
            ###############################
        ...
        for i in range(k):
            bess_basis_tmp += [
                sym.simplify(normalizer[order][i] *
                             f[order].subs(x, zeros[order, i] * x / cutoff))
            ]
        ...
        #################
            ...
            if not zero_m_only:
                for i in range(1, k):
                    ...
                    ##############################################
                    P_l_m[i][i] = sym.simplify((1 - 2 * i) * P_l_m[i - 1][i - 1]*sym.sqrt(1-z**2))
                    ##############################################
                    if i + 1 < k:
...
class dist_emb(torch.nn.Module):
    def __init__(self, num_radial, cutoff=5.0, envelope_exponent=5):
        ...
        self.freq = torch.nn.Parameter(torch.Tensor(num_radial)).to(device)
    ...
    def forward(self, dist):
        ...
        # return (self.freq * dist).sin()/(dist*self.cutoff)
...
class angle_emb(torch.nn.Module):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
        ...
        bessel_forms = bessel_basis(num_spherical, num_radial, cutoff)
...
class torsion_emb(torch.nn.Module):
    def __init__(self, num_spherical, num_radial, cutoff=5.0,
        ...
        for i in range(self.num_spherical):
            if i == 0:
                ...
                sph2 = []
                for k in range(-i, i + 1):
                    sph2.append(sym.lambdify([theta, phi], sph_harm_forms[i][k+i], modules))
                self.sph_funcs.append(sph2)
            bessel2 = []
            for j in range(self.num_radial):
                bessel2.append(sym.lambdify([x], bessel_forms[i][j], modules))
            self.bessel_funcs.append(bessel2)
    ...
    def forward(self, dist, angle, phi, idx_kj):
        ...
        for i in range(self.num_spherical):
            for k in range(-i, i + 1):
                for j in range(self.num_radial):
                    out.append(self.bessel_funcs[i][j](dist)[idx_kj]*self.sph_funcs[i][k](angle, phi))
